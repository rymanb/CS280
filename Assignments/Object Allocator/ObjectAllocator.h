...
class ObjectAllocator
{
  ...
  private:
	...
	OAConfig Config_;                   // the configuration parameters
	OAStats Stats_;                     // the statistics for the allocator
	size_t HeaderSize_;                 // size of the header block
	size_t InnerSize_;                   // size of the data block
	...
	bool IsUsed(GenericObject* object) const;
	bool IsPageEmpty(GenericObject* page) const;

	void FreePage(GenericObject* page);

	void InitializeHeaderBlock(GenericObject* object, OAConfig::HBLOCK_TYPE headerType, const char* label_);
	void FreeHeaderBlock(GenericObject* object, OAConfig::HBLOCK_TYPE headerType);
	
	void CheckBoundaries(unsigned char* address) const;
	
	unsigned char* GetLeftAlignBlock(GenericObject* object) const;
	unsigned char* GetRightAlignBlock(GenericObject* object) const;
